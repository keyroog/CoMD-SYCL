...
{
   // Keep data device-resident: GPU reduction writes into s->gpu.d_eLocal,
   // OneCCL allreduce runs directly on device, one D2H at the very end.
   startTimer(commReduceTimer);
   computeEnergyDevice(s, s->gpu.d_eLocal);
   addRealParallelDevice(s->gpu.d_eLocal, s->gpu.d_eSum, 2);
   stopTimer(commReduceTimer);

   real_t eSum[2];
   dpct::get_in_order_queue().memcpy(eSum, s->gpu.d_eSum, 2 * sizeof(real_t)).wait();
   s->ePotential = eSum[0];
   s->eKinetic   = eSum[1];
}
...
void redistributeAtomsGpuNL(SimFlat* sim)
{
   ...
   free(nAtomsGPU);
#endif
   // Allreduce directly on the device flag — no D2H before the collective,
   // one D2H after to read the result on CPU.
   addIntParallelDevice(sim->gpu.d_updateLinkCellsRequired,
                        sim->gpu.d_updateLinkCellsResult, 1);
   int flag = 0; //indicates if any particle has migrated from one processor to the other
   dpct::get_in_order_queue()
       .memcpy(&flag, sim->gpu.d_updateLinkCellsResult, sizeof(int))
       .wait();
   ...
   if(flag > 0){
           ...
           dpct::get_in_order_queue()
               .memset(sim->gpu.d_updateLinkCellsRequired, 0, sizeof(int))
               .wait();
   }
