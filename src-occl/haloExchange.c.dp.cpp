...
HaloExchange* initAtomHaloExchange(Domain* domain, LinkCell* boxes)
{
   ...
   // Allocate pinned (page-locked) host memory for DMA-friendly GPU transfers.
   // This replaces comdMalloc + cudaHostRegister from the original CUDA code.
   hh->sendBufM = (char*)sycl::malloc_host(hh->bufCapacity, dpct::get_in_order_queue());
   hh->sendBufP = (char*)sycl::malloc_host(hh->bufCapacity, dpct::get_in_order_queue());
   hh->recvBufP = (char*)sycl::malloc_host(hh->bufCapacity, dpct::get_in_order_queue());
   hh->recvBufM = (char*)sycl::malloc_host(hh->bufCapacity, dpct::get_in_order_queue());
   ...
   for (int ii=0; ii<6; ++ii) {
      ...
      parms->cellListGpu[ii] = sycl::malloc_device<int>(
          parms->nCells[ii], dpct::get_in_order_queue());
      dpct::get_in_order_queue()
          .memcpy(parms->cellListGpu[ii], parms->cellList[ii],
                  parms->nCells[ii] * sizeof(int))
          .wait();
   }
   // allocate scan buf
   ...
   parms->d_natoms_buf =
       sycl::malloc_device<int>(size, dpct::get_in_order_queue());
   parms->h_natoms_buf = (int*) malloc( size * sizeof(int));
   parms->d_partial_sums =
       sycl::malloc_device<int>(size, dpct::get_in_order_queue());
...
HaloExchange* initForceHaloExchange(Domain* domain, LinkCell* boxes, int useGPU)
{
   ...
   for (int ii=0; ii<6; ++ii)
   {
      ...
      // copy cell list to gpu
      parms->sendCellsGpu[ii] = sycl::malloc_device<int>(
          parms->nCells[ii], dpct::get_in_order_queue());
      parms->recvCellsGpu[ii] = sycl::malloc_device<int>(
          parms->nCells[ii], dpct::get_in_order_queue());
      dpct::get_in_order_queue()
          .memcpy(parms->sendCellsGpu[ii], parms->sendCells[ii],
                  parms->nCells[ii] * sizeof(int))
          .wait();
      dpct::get_in_order_queue()
          .memcpy(parms->recvCellsGpu[ii], parms->recvCells[ii],
                  parms->nCells[ii] * sizeof(int))
          .wait();
...
void destroyHaloExchange(HaloExchange** haloExchange)
{
   ...
   sycl::free((*haloExchange)->sendBufM, dpct::get_in_order_queue());
   sycl::free((*haloExchange)->sendBufP, dpct::get_in_order_queue());
   sycl::free((*haloExchange)->recvBufP, dpct::get_in_order_queue());
   sycl::free((*haloExchange)->recvBufM, dpct::get_in_order_queue());
...
//           printf("gpu: %d\n",nTotalAtomsCellList);
           ...
           /*
           DPCT1124:110: cudaMemcpyAsync is migrated to asynchronous memcpy API.
           ...
           */
           sim->boundary_stream->memcpy(charBuf, (void *)(sim->gpu_atoms_buf),
                                        nTotalAtomsCellList * sizeof(AtomMsg));
           sim->boundary_stream->wait();
   }
...
void destroyAtomsExchange(void* vparms)
{
   ...
   for (int ii=0; ii<6; ++ii)
   {
      ...
      dpct::dpct_free(parms->cellListGpu[ii], dpct::get_in_order_queue());
   }
   dpct::dpct_free(parms->d_natoms_buf, dpct::get_in_order_queue());
   dpct::dpct_free(parms->d_partial_sums, dpct::get_in_order_queue());
...
void destroyForceExchange(void* vparms)
{
   ...
   for (int ii=0; ii<6; ++ii)
   {
      ...
      dpct::dpct_free(parms->sendCellsGpu[ii], dpct::get_in_order_queue());
      dpct::dpct_free(parms->recvCellsGpu[ii], dpct::get_in_order_queue());
      dpct::dpct_free(parms->natoms_buf[ii], dpct::get_in_order_queue());
      dpct::dpct_free(parms->partial_sums[ii], dpct::get_in_order_queue());
   }
}
