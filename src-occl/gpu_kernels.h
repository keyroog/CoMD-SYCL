...
EXTERN_C void updateNeighborsGpuAsync(SimGpu sim, int *temp, int nCells,
...
// latency hiding opt
EXTERN_C void eamForce1GpuAsync(SimGpu sim, AtomListGpu atoms_list,
                                int num_cells, int *cells_list, int method,
                                dpct::queue_ptr stream, int spline);
EXTERN_C void eamForce2GpuAsync(SimGpu sim, AtomListGpu atoms_list,
                                int num_cells, int *cells_list, int method,
                                dpct::queue_ptr stream, int spline);
EXTERN_C void eamForce3GpuAsync(SimGpu sim, AtomListGpu atoms_list,
                                int num_cells, int *cells_list, int method,
                                dpct::queue_ptr stream, int spline);

EXTERN_C void emptyNeighborListGpu(SimGpu * sim, int boundaryFlag);

EXTERN_C int compactCellsGpu(char *work_d, int nCells, int *d_cellList,
                             SimGpu sim_gpu, int *d_cellOffsets,
                             int *d_workScan, real3_old shift,
                             dpct::queue_ptr stream);
EXTERN_C void unloadAtomsBufferToGpu(char *buf, int nBuf, SimFlat *s,
                                     char *gpu_buf, dpct::queue_ptr stream);
EXTERN_C void loadForceBufferFromGpu(char *buf, int *nbuf, int nCells,
                                     int *cellList, int *natoms_buf,
                                     int *partial_sums, SimFlat *s,
                                     char *gpu_buf, dpct::queue_ptr stream);
EXTERN_C void unloadForceBufferToGpu(char *buf, int nBuf, int nCells,
                                     int *cellList, int *natoms_buf,
                                     int *partial_sums, SimFlat *s,
                                     char *gpu_buf, dpct::queue_ptr stream);
...
// variant that writes into a caller-supplied device buffer (no D2H copy)
EXTERN_C void computeEnergyDevice(SimFlat *sim, real_t *d_eOut);

EXTERN_C void advanceVelocityGpu(SimGpu sim, real_t dt);
EXTERN_C void advancePositionGpu(SimGpu* sim, real_t dt);

EXTERN_C void buildAtomListGpu(SimFlat *sim, dpct::queue_ptr stream);
EXTERN_C void updateLinkCellsGpu(SimFlat *sim);
EXTERN_C void sortAtomsGpu(SimFlat *sim, dpct::queue_ptr stream);

EXTERN_C int neighborListUpdateRequiredGpu(SimGpu* sim);
EXTERN_C void updateNeighborsGpuAsync(SimGpu sim, int *temp, int num_cells,
                                      int *cell_list, dpct::queue_ptr stream);
