...
void updateGpuBoundaryCells(SimFlat *sim)
{
      for(int i = 0; i < sim->n_boundary1_cells; ++i)
      {
              ...
              dpct::get_in_order_queue().memcpy(
                  (sim->gpu.atoms.r.x) + (iBox * MAXATOMS),
                  (sim->host.atoms.r.x) + (iBox * MAXATOMS), f_size);
              dpct::get_in_order_queue().memcpy(
                  (sim->gpu.atoms.r.y) + (iBox * MAXATOMS),
                  (sim->host.atoms.r.y) + (iBox * MAXATOMS), f_size);
              dpct::get_in_order_queue().memcpy(
                  (sim->gpu.atoms.r.z) + (iBox * MAXATOMS),
                  (sim->host.atoms.r.z) + (iBox * MAXATOMS), f_size);
              dpct::get_in_order_queue().memcpy(
                  (sim->gpu.atoms.p.x) + (iBox * MAXATOMS),
                  (sim->host.atoms.p.x) + (iBox * MAXATOMS), f_size);
              dpct::get_in_order_queue().memcpy(
                  (sim->gpu.atoms.p.y) + (iBox * MAXATOMS),
                  (sim->host.atoms.p.y) + (iBox * MAXATOMS), f_size);
              dpct::get_in_order_queue().memcpy(
                  (sim->gpu.atoms.p.z) + (iBox * MAXATOMS),
                  (sim->host.atoms.p.z) + (iBox * MAXATOMS), f_size);

              dpct::get_in_order_queue().memcpy(
                  (sim->gpu.atoms.gid) + (iBox * MAXATOMS),
                  (sim->atoms->gid) + (iBox * MAXATOMS), i_size);
              dpct::get_in_order_queue()
                  .memcpy((sim->gpu.atoms.iSpecies) + (iBox * MAXATOMS),
                          (sim->atoms->iSpecies) + (iBox * MAXATOMS), i_size)
                  .wait();
      }
}
