...
/// Categoria A (device data): CUDA-aware MPI directly on device pointers (GPUDirect).
...
// --- Categoria A: device-resident data, CUDA-aware MPI (GPUDirect) --------

/// CUDA-aware MPI allreduce integer sum directly on device pointers.
void addIntParallelDevice(int* d_sendBuf, int* d_recvBuf, int count);

/// CUDA-aware MPI allreduce real sum directly on device pointers.
